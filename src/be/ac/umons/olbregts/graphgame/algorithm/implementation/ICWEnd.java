 ...
 */
public class ICWEnd implements Algorithm {
    ...
    public void reset(Game game) throws IllegalGraphException {
        ...
        for (String vertexId : game.getGraph().getVertexsId()) {
            int[] cValues = new int[this.game.getWindowsSize() + 1];
            if (this.game.getTargetVertexs().contains(vertexId)) {
                cValues[0] = 0;
            } else {
                cValues[0] = Integer.MIN_VALUE;
            }
            c.put(vertexId, cValues);
